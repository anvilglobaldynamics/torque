...
class Collection {
  ...
  // ================== Lipi specific features ================== //

  async autoGenerateOrganizationSpecificNumber({ organizationId, fieldName }) {
    if (!organizationId) throw new CodedError("DEV_ERROR", "autoGenerateOrganizationSpecificNumber() -> organizationId Missing");
    if (!fieldName) throw new CodedError("DEV_ERROR", "autoGenerateOrganizationSpecificNumber() -> fieldName Missing");
    const autoGeneratedSalesNumberCollectionName = 'auto-generated-organization-specific-number';
    let query = {
      organizationId
    };
    let modifications = {
    ...
    let doc = await this._db.upsertAndReturnNew(autoGeneratedSalesNumberCollectionName, query, modifications);
    if (!doc) {
      throw new CodedError("INTERNAL_DATABASE_ERROR", `Unable to auto generate organization specific ${fieldName}`);
    }
