...
exports.SalesCollection = class extends Collection {
  ...
  // NOTE: This method creates a serial number for sales unique to the
  // organization. At a later time, if we wish to implement a similar
  // behavior for other collections, we can refactor the method into
  // a new collection/mixin.
  async _autoGenerateSalesNumber({ organizationId }) {
    const autoGeneratedSalesNumberCollectionName = 'auto-generated-sales-number';
    let query = {
      organizationId
    };
    let modifications = {
    ...
    if (!doc) {
      throw new CodedError("INTERNAL_DATABASE_ERROR", "Unable to auto generate salesNumber");
    }
    return doc['salesNumberSeed'];
  }
  ...
  async create({ organizationId, outletId, customerId, productList, serviceList, payment, assistedByEmployeeId, wasOfflineSale = false, productsSelectedFromWarehouseId }) {
    let salesNumber = await this._autoGenerateSalesNumber({ organizationId });
