...
exports.OrganizationSettingsCollection = class extends Collection {
  ...
  async create({ organizationId, receiptText1, receiptText2, logoImageId, monetaryUnit, decimanFormatPreset }) {
    return await this._insert({
      organizationId, receiptText1, receiptText2, logoImageId, monetaryUnit, decimanFormatPreset,
      isDeleted: false
    });
  }

  async setDetailsByOrganizationId({ organizationId }, { receiptText1, receiptText2, logoImageId, monetaryUnit, decimanFormatPreset }) {
    return await this._update({ organizationId }, {
      $set: {
        receiptText1, receiptText2, logoImageId, monetaryUnit, decimanFormatPreset
      }
    });
  }
