...
class DatabaseService {
  ...
  async insertMany(collectionName, docList) {
    let results = await this._db.collection(collectionName).insertMany(doc);
  ...
  async deleteOne(collectionName, query, cbfn) {
    let results = await this._db.collection(collectionName).deleteOne(query);
    return (results.deletedCount === 1);
  }

  async deleteMany(collectionName, query, cbfn) {
    let results = await this._db.collection(collectionName).deleteMany(query);
    return results.deletedCount;
  }

  async findByNativeId(collectionName, _id, cbfn) {
    return await this.findOne(collectionName, { _id: ObjectId(_id) }, cbfn);
  }
  ...
  async _autoGenerateKey(collectionName, cbfn) {
    let query = {
      which: 'only'
    };
    let modifications = {
    ...
    let doc = this.updateAndReturnNew(this._autoGeneratedKeyCollectionName, query, modifications);
    if (!doc) {
