...
class Database {
  ...
  constructor(path) {
    ...
    this.__isAutogenerationBusy = false;
    this.__autogenerationQueue = [];
  }
  ...
  */
  ...
  __autogenerateKey(collectionName, cbfn) {
  ...
  __autogenerateNextInQueue() {
    if (this.__isAutogenerationBusy) return;
    if (this.__autogenerationQueue.length === 0) return;
    let { collectionName, cbfn } = this.__autogenerationQueue.shift();
    this.__isAutogenerationBusy = true;
    this.__autogenerateKey(collectionName, (...args) => {
      cbfn(...args);
      this.__isAutogenerationBusy = false;
      this.__autogenerateNextInQueue();
    });
  }

  autoGenerateKey(collectionName, cbfn) {
    this.__autogenerationQueue.push({ collectionName, cbfn });
    this.__autogenerateNextInQueue();
  }
