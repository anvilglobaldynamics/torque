...
class DatabaseEngine {
  ...
  constructor({ path, name }) {
    ...
    this._autoGeneratedKeyCollectionName = 'auto-generated-keys'; // FIXME: Change name to auto-generated-keys
    this._db = null;
  }
  ...
  async initialize() {
    const client = await MongoClient.connect(this._mongodbPath);
    const db = client.db(this._mongodbDatabaseName);
    this._db = db;
    await this.__initializeAutoGeneratedKeyIndex();
  }
  ...
  async insertOne(collectionName, doc) {
    ...
    if (results.insertedCount === 1) return doc.id;
    return null;
  }
  ...
  async upsertAndReturnNew(collectionName, query, modifications) {
    let results = await this._db.collection(collectionName).findOneAndUpdate(query, modifications, { upsert: true, returnOriginal: false });
    return (results.value || null);
  }
